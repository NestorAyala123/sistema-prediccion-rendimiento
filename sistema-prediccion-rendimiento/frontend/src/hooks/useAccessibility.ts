...
function removeEventListeners(element: HTMLElement, events: string[]): void {
  if (!(element instanceof HTMLElement)) return;
  
  const eventListeners: EventListenerInfo[] = [];

  const addToList = (type: string, handler: EventListener, elem: HTMLElement): void => {
  ...
  try {
    ...
    if (s.largeButtons) {
      ...
      document.querySelectorAll(buttonSelector).forEach(button => {
        if (button instanceof HTMLElement) {
          button.dataset.originalSize = 'true';
        }
      });
    } else {
      ...
      document.querySelectorAll(`${buttonSelector}[data-original-size]`).forEach(button => {
        if (button instanceof HTMLElement) {
          delete button.dataset.originalSize;
        }
      });
    }

    // Control de scroll y animaciones
    if (s.blockAutoScroll) {
    ...
    document.querySelectorAll('video, audio').forEach(media => {
      if (media instanceof HTMLMediaElement) {
        media.controls = s.audioControls || false;
        
        Array.from(media.textTracks).forEach(track => {
          if (track.kind === 'subtitles' || track.kind === 'captions') {
            track.mode = s.textToSpeech ? 'showing' : 'disabled';
          }
        });
      }
    });
    ...
    if (s.visualAlerts) {
      document.querySelectorAll('audio').forEach(audio => {
        if (audio instanceof HTMLAudioElement && !audio.dataset.visualAlertHandlerAdded) {
          const handlePlay = () => {
      ...
      document.querySelectorAll('video').forEach(video => {
        if (video instanceof HTMLVideoElement) {
          Array.from(video.textTracks).forEach(track => {
            // Buscar pistas de tipo 'metadata' con etiqueta que indique lenguaje de señas
            if (track.kind === 'metadata' && track.label.toLowerCase().includes('sign')) {
              track.mode = 'showing';
            }
          });
        }
